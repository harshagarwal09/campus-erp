...
export default function ViewFestMembersPage() {
  ...
  const fetchMembers = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/fests/${id}/members`, { credentials: 'include' });
  ...
  const handleAssign = async () => {
    ...
    setAssigning(true);
    try {
      const res = await fetch(`/api/fests/${id}/members`, {
        method: 'POST',
        credentials: 'include',
  ...
  const handleRemove = async (studentId: number) => {
    ...
    try {
      const res = await fetch(`/api/fests/${id}/members?userId=${studentId}`, {
        method: 'DELETE',
        credentials: 'include',
      });
      const data = await res.json();
  ...
  const roleBadgeColor = (role: string) => {
    if (role === 'Overall Coordinator') return { background: '#eff6ff', color: '#2563eb' };
    if (role.includes('Lead')) return { background: '#fef3c7', color: '#b45309' };
      ...
      <div className="page">
        <div className="page-header">
          <div>
            ...
            <h1>Fest Participants</h1>
            <p className="subtitle">Manage participants of this fest.</p>
          </div>
          <div className="page-header-actions">
            <button className="secondary" onClick={() => router.push(`/admin/fests/${id}/view`)}>Back</button>
