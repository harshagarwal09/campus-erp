      ...
      .then((data) => {
        if (data.success) {
          ...
          setForm({
            ...
            maxParticipants: d.maxParticipants ? String(d.maxParticipants) : '',
          });
        } else {
          setLoadError(data.error || 'Hackathon not found');
        }
      })
    ...
    try {
      ...
      if (data.success) {
        ...
        const firstErr = data.data?.errors
          ...
          : data.error;
        showToast('error', firstErr || 'Failed to update hackathon');
      }
    } catch {
      ...
      <div className="page">
        <div className="page-header" style={{ alignItems: 'flex-start' }}>
          <div>
            ...
            <p className="subtitle">Update the hackathon details below.</p>
          </div>
          <button className="secondary" onClick={() => router.push('/admin/hackathons')}>Back</button>
        </div>
