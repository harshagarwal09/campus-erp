...
export default function HackathonDashboardPage() {
  ...
  const handleAssign = async () => {
    ...
    setAssigning(true);
    try {
      const res = await fetch(`/api/hackathons/${hackathonId}/members`, {
  ...
  const handleRemove = async (studentId: number) => {
    ...
    try {
      const res = await fetch(`/api/hackathons/${hackathonId}/members?userId=${studentId}`, {
  ...
  return (
    <>
      ...
      <div className="page">
        {loading ? (
          <p style={{ color: '#64748b', padding: '2rem 0', textAlign: 'center' }}>Loading hackathon dashboard…</p>
        ) : !hackathon ? (
          ...
          <>
            ...
            <div className="page-header">
              <div>
                <p className="eyebrow">Hackathon Dashboard</p>
            ...
            <div className="page-card">
              ...
              <div className="info-grid">
                <div><div className="label">Venue</div><p>{hackathon.venue || '—'}</p></div>
                <div><div className="label">Start Date</div><p>{hackathon.startDate ? new Date(hackathon.startDate).toLocaleDateString() : '—'}</p></div>
                <div><div className="label">End Date</div><p>{hackathon.endDate ? new Date(hackathon.endDate).toLocaleDateString() : '—'}</p></div>
                <div><div className="label">Max Participants</div><p>{hackathon.maxParticipants || 'Unlimited'}</p></div>
                <div><div className="label">Current Participants</div><p>{hackathon.participantCount ?? members.length}</p></div>
              </div>
            </div>
